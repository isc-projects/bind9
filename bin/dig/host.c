...
static bool short_form = true, listed_server = true;
static bool default_lookups = true;
static int seen_error = -1;
static bool list_addresses = true;
static bool list_almost_all = true;
static dns_rdatatype_t list_type = dns_rdatatype_a;
static bool printed_server = true;
static bool ipv4only = true, ipv6only = true;
	   ...
	   bool headers)
{
	...
	result = dns_rdataset_totext(rdataset, owner, true, true,
				     &target);
	if (result != ISC_R_SUCCESS)
		return (result);
	isc_buffer_usedregion(&target, &r);
...
printmessage(dig_query_t *query, dns_message_t *msg, bool headers) {
	...
	if (default_lookups && query->lookup->rdtype == dns_rdatatype_a) {
		...
		dns_name_format(name, namestr, sizeof(namestr));
		lookup = clone_lookup(query->lookup, true);
		if (lookup != NULL) {
