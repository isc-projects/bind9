...
n=`expr $n + 1`
echo_i "adding dom5.example with 'masters' suboption set and a random label ($n)"
ret=0
$NSUPDATE -d <<END >> nsupdate.out.test$n 2>&1 || ret=1
    server 10.53.0.1 ${PORT}
...
n=`expr $n + 1`
echo_i "adding dom6.example and global masters option ($n)"
ret=0
$NSUPDATE -d <<END >> nsupdate.out.test$n 2>&1 || ret=1
    server 10.53.0.1 ${PORT}
...
n=`expr $n + 1`
echo_i "adding domain dom9.example to catalog1 zone with masters and tsig key($n)"
ret=0
$NSUPDATE -d <<END >> nsupdate.out.test$n 2>&1 || ret=1
    server 10.53.0.1 ${PORT}
...
n=`expr $n + 1`
echo_i "reconfiguring slave - removing catalog4 catalog zone ($n)"
ret=0
