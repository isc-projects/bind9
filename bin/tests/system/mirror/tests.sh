...
reload_zone verify-axfr ${UPDATED_SERIAL_BAD}
...
# Ensure a delegation is returned.
...
# Issue a non-recurisve query for something below the cached zone cut.
$DIG $DIGOPTS @10.53.0.3 foo.sub.example. A +norec > dig.out.ns3.test$n.2 2>&1 || ret=1
# Ensure the same delegation is returned.
