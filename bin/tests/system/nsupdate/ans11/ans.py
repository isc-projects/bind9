...
def sig_rdata(covers: dns.rdatatype.RdataType, ttl: int) -> dns.rdata.Rdata:
    """
    ...
    text = f"{covered} 6 2 {ttl} 20260331170000 20260318160000 21831 . 0000"
...
def sig(covers: dns.rdatatype.RdataType, ttl: int) -> dns.rrset.RRset:
    return dns.rrset.from_rdata(HOST, ttl, sig_rdata(covers, ttl))
...
class SigAxfrHandler(QnameQtypeHandler, AxfrHandler):
    ...
    covered types (A, MX) and TTLs (600, 1200) differ; the transferring
    secondary must keep them as separate (SIG, A) and (SIG, MX)
    rdatasets with their distinct TTLs (GL#5818).
    """
    ...
    zone_contents = [
        ...
        sig(dns.rdatatype.A, 600),
        sig(dns.rdatatype.MX, 1200),
    ]
    final_soa = soa()
