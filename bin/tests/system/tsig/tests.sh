...
then
  ...
  ret=0
  $KEYGEN -r $RANDFILE -a dh -b 2048 -n host example.net > keygen.out1 2>&1 || ret=1
  grep dns_dnssec_findmatchingkeys keygen.out1 > /dev/null && ret=1
  $KEYGEN -r $RANDFILE -a dh -b 2048 -n host example.net > keygen.out2 2>&1 || ret=1
...
ret=0
$KEYGEN -r $RANDFILE -a hmac-sha256 -b 2048 -n host example.net > keygen.out3 2>&1 && ret=1
