...
sed -n '/plain success\/timeout/,/Unassociated entries/p' ns2/named_dump.1.db | grep 'ns.flushtest.example' > /dev/null 2>&1 || ret=1
$RNDC $RNDCOPTS flushtree flushtest.example || ret=1
dump_cache
mv ns2/named_dump.db ns2/named_dump.2.db
sed -n '/plain success\/timeout/,/Unassociated entries/p' ns2/named_dump.2.db | grep 'ns.flushtest.example' > /dev/null 2>&1 && ret=1
if [ $ret != 0 ]; then echo "I:failed"; fi
status=`expr $status + $ret`
