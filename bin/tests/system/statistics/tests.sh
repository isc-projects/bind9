...
ret=0
echo_i "checking for 2 entries in adb hash table"
...
echo_i "dumping updated stats for ns3"
rm -f ns3/named.stats
$RNDCCMD -s 10.53.0.3 stats > /dev/null 2>&1
[ -f ns3/named.stats ] || ret=1
if [ $ret != 0 ]; then echo_i "failed"; fi
...
echo_i "verifying queries in progress"
...
ret=0
echo_i "verifying bucket size output"
...
ret=0
n=`expr $n + 1`
echo_i "checking that zones with slash are properly shown in XML output (${t})"
