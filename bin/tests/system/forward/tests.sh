...
{ $PERL ../digcomp.pl dig.out.hidden dig.out.f1 || ret=1; } | cat_i
...
{ $PERL ../digcomp.pl dig.out.root dig.out.f1 || ret=1; } | cat_i
if [ $ret != 0 ]; then echo_i "failed"; fi
status=`expr $status + $ret`
...
{ $PERL ../digcomp.pl dig.out.hidden dig.out.f2 || ret=1; } | cat_i
if [ $ret != 0 ]; then echo_i "failed"; fi
status=`expr $status + $ret`
...
{ $PERL ../digcomp.pl dig.out.root dig.out.f2 || ret=1; } | cat_i
if [ $ret != 0 ]; then echo_i "failed"; fi
status=`expr $status + $ret`
