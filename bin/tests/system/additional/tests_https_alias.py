...
def _additional(res, target):
    ...
    With "minimal-responses no", ns3 also adds the delegation NS RRset
    to the authority section and the address of its name server to the
    additional section; that address record is not part of the HTTPS
    additional processing and is skipped.
    """
    ...
    additional = [rrset for rrset in res.additional if rrset.name not in nameservers]
    assert all(rrset.name == owner for rrset in additional), res.additional
    return {rrset.rdtype: rrset for rrset in additional}
