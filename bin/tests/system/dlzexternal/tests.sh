...
ret=0
echo_i "testing DLZ driver is cleaned up on reload"
...
ret=0
echo_i "testing multiple DLZ drivers"
test_update testdc1.alternate.nil. A "86400 A 10.53.0.10" "10.53.0.10" || ret=1
status=`expr $status + $ret`

ret=0
echo_i "testing AXFR from DLZ drivers"
...
ret=0
echo_i "testing unsearched/unregistered DLZ zone is not found"
...
ret=0
echo_i "testing unsearched/registered DLZ zone is found"
...
ret=0
echo_i "testing correct behavior with findzone returning ISC_R_NOMORE"
...
ret=0
echo_i "testing findzone can return different results per client"
