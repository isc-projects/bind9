...
newtest "check root-key-sentinel-not-ta with old ta, without DO=1 or AD=1 and" " 'root-key-sentinel yes;' (expect NOERROR)"
$DIG $DIGOPTS +noad +nodnssec @10.53.0.3 root-key-sentinel-not-ta-${oldid}.example A > dig.out.ns3.test$n || ret=1
grep "status: NOERROR" dig.out.ns3.test$n > /dev/null || ret=1
if [ $ret != 0 ]; then echo_i "failed"; fi
status=`expr $status + $ret`
...
newtest "check root-key-sentinel-is-ta with new ta, without DO=1 or AD=1 and" " 'root-key-sentinel yes;' (expect NOERROR)"
$DIG $DIGOPTS +noad +nodnssec @10.53.0.3 root-key-sentinel-is-ta-${newid}.example A > dig.out.ns3.test$n || ret=1
grep "status: NOERROR" dig.out.ns3.test$n > /dev/null || ret=1
if [ $ret != 0 ]; then echo_i "failed"; fi
status=`expr $status + $ret`
...
newtest "check root-key-sentinel-is-ta with bad ta, without DO=1 or AD=1 and" " 'root-key-sentinel yes;' (expect NXDOMAIN)"
$DIG $DIGOPTS +noad +nodnssec @10.53.0.3 root-key-sentinel-is-ta-${badid}.example A > dig.out.ns3.test$n || ret=1
