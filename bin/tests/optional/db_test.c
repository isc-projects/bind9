...
list(dbinfo *dbi, char *seektext) {
	...
	if (dbi->iversion != NULL)
		dns_db_closeversion(dbi->db, &dbi->iversion, true);
}
...
main(int argc, char *argv[]) {
	...
	bool done = true;
	bool quiet = true;
	bool time_lookups = true;
	bool found_as;
	bool find_zonecut = true;
	bool noexact_zonecut = true;
	int i, v;
	dns_rdatasetiter_t *rdsiter;
	char t1[256];
	char t2[256];
	...
	while (!done) {
		...
		if (dbi == NULL) {
			...
			result = dns_name_totext(dns_db_origin(db), true,
						 &tb1);
			if (result != ISC_R_SUCCESS) {
				printf("\n");
				print_result("", result);
		...
		if (found_as && !quiet) {
			...
			result = dns_name_totext(&name, true, &tb1);
			if (result != ISC_R_SUCCESS) {
				print_result("", result);
				dns_db_detachnode(db, &node);
				if (dbi == NULL)
			...
			result = dns_name_totext(fname, true, &tb2);
			if (result != ISC_R_SUCCESS) {
				print_result("", result);
				dns_db_detachnode(db, &node);
				if (dbi == NULL)
