...
1. **Scope the cycle.** Range = the most recent `Set up version for BIND
   ...
   `python3 scripts/cycle_merges.py`. (Action/audience taxonomy: REFERENCE.md.)
...
4. **Issue close-status check.** Derive the full covered-issue set
   (`python3 scripts/issue_refs.py <ver>`), fetch each via gitlab CLI,
   then `python3 scripts/summarize_issues.py <dir>`. Every covered issue should be
