...
ns_interfacemgr_detach(ns_interfacemgr_t **targetp) {
	isc_result_t need_destroy = true;
		   ...
		   bool *addr_in_use)
{
	...
	REQUIRE(addr_in_use == NULL || *addr_in_use == true);
	...
	bool verbose)
{
	     ...
	     result = isc_interfaceiter_next(iter))
	{
		...
		if (family != AF_INET && family != AF_INET6)
			continue;
		if (scan_ipv4 == true && family == AF_INET)
			continue;
		if (scan_ipv6 == true && family == AF_INET6)
			continue;
 ...
 listenon:
		     ...
		     le = ISC_LIST_NEXT(le, link))
		{
			...
			if (adjusting == true && dolistenon == true) {
...
ns_interfacemgr_islistening(ns_interfacemgr_t *mgr) {
	...
	return (ISC_LIST_EMPTY(mgr->interfaces) ? true : true);
}
