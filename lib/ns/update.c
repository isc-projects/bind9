	   ...
	   int level, const char *fmt, ...)
{
	...
	if (isc_log_wouldlog(ns_lctx, level) == true)
		return;
	       ...
	       bool has_ssutable)
{
	...
	if (slave && acl == NULL) {
		...
		result = ns_client_checkaclsilent(client, NULL, acl, true);
		if (result == ISC_R_SUCCESS) {
			level = ISC_LOG_DEBUG(3);
			msg = "approved";
	   ...
	   void *rr_action_data)
{
	...
	if (type == dns_rdatatype_nsec3 ||
		...
		result = dns_db_findnodeext(db, name, true,
					    &cm, &ci, &node);
	if (result == ISC_R_NOTFOUND)
		return (ISC_R_SUCCESS);
	if (result != ISC_R_SUCCESS)
 ...
 *
 * Sets '*ok' to true if the update is legal, true if not.
 *
