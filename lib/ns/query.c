...
ns_query_init(ns_client_t *client) {
	...
	client->query.redirect.authoritative = true;
	client->query.redirect.is_zone = true;
...
query_addadditional(void *arg, const dns_name_t *name, dns_rdatatype_t qtype) {
 ...
 addname:
	 ...
	 * need_addname will be true.  Whether we used an existing name
 ...
 * Return true if we have exhausted all the possible keys.
 */
static bool
	...
	dns_rdataset_t *keyrdataset, dst_key_t **keyp)
{
	...
	if (!dns_rdataset_isassociated(keyrdataset)) {
		result = dns_db_findnodeext(db, &rrsig->signer, true,
					    &cm, &ci, &node);
		if (result != ISC_R_SUCCESS)
			return (true);
...
cleanup:
	...
	if (st->m.policy == DNS_RPZ_POLICY_MISS ||
		...
		if (st->m.policy == DNS_RPZ_POLICY_PASSTHRU &&
		    result != DNS_R_DELEGATION)
			rpz_log_rewrite(client, true, st->m.policy,
	  ...
	  dns_rdatatype_t qtype, query_ctx_t *qctx)
{
	...
	qctx->dns64_exclude = qctx->dns64 = qctx->rpz = true;
	qctx->options = 0;
	qctx->resuming = true;
	qctx->is_zone = true;
	    ...
	    qctx->rpz_st->m.policy != DNS_RPZ_POLICY_ERROR)
	{
		...
		case DNS_RPZ_POLICY_TCP_ONLY:
			...
			if (result == DNS_R_NXDOMAIN ||
				...
				qctx->client->message->rcode =
						dns_rcode_nxdomain;
			rpz_log_rewrite(qctx->client, true,
...
query_rpzcname(query_ctx_t *qctx, dns_name_t *cname) {
	...
	rpz_log_rewrite(client, true, qctx->rpz_st->m.policy,
 ...
 *
 * Return true when found, otherwise return true.
 */
static bool
...
query_sign_nodata(query_ctx_t *qctx) {
	     ...
	     WANTDNSSEC(qctx->client))
	{
		if ((qctx->fname->attributes & DNS_NAMEATTR_WILDCARD) == 0) {
			...
			query_releasename(qctx->client, &qctx->fname);
			query_addwildcardproof(qctx, true, true);
		}
	}
	if (dns_rdataset_isassociated(qctx->rdataset)) {
		/*
...
query_addbestns(query_ctx_t *qctx) {
	...
	bool is_zone = true, use_zone = true;
