...
query_additional(query_ctx_t *qctx, dns_rdataset_t *rdataset) {
	...
	CALL_HOOK(NS_QUERY_ADDITIONAL_BEGIN, qctx);
...
query_checkrrl(query_ctx_t *qctx, isc_result_t result) {
	...
	} else if (!TCP(qctx->client) && qctx->view->requireservercookie &&
...
query_done(query_ctx_t *qctx) {
	...
	if (qctx->want_stale) {
		...
		if (result == ISC_R_SUCCESS && stale_ttl > 0)  {
			switch (qctx->view->staleanswersok) {
			case dns_stale_answer_yes:
				staleanswersok = true;
				break;
			case dns_stale_answer_conf:
				staleanswersok = qctx->view->staleanswersenable;
				break;
