...
#define atomic_store_explicit32(obj, desired, order)		\
	(order == memory_order_relaxed				\
	 ? InterlockedExchangeNoFence(obj, desired)		\
	 : (order == memory_order_acquire			\
	   ? InterlockedExchangeAcquire(obj, desired)		\
	    : InterlockedExchange(obj, desired)))

#define atomic_store_explicit64(obj, desired, order)		\
	(order == memory_order_relaxed				\
	 ? InterlockedExchange64NoFence(obj, desired)		\
	 : (order == memory_order_acquire			\
	    ? InterlockedExchange64Acquire(obj, desired)	\
...
#define atomic_load_explicit32(obj, order)			\
	(order == memory_order_relaxed			\
	 ? (int32_t)InterlockedOrNoFence((int32_t volatile *)obj, 0)	\
	 : (order == memory_order_acquire			\
	    ? (int32_t)InterlockedOrAcquire((int32_t volatile *)obj, 0)	\
	    : (order == memory_order_release			\
	       ? (int32_t)InterlockedOrRelease((int32_t volatile *)obj, 0) \
	       : (int32_t)InterlockedOr((int32_t volatile *)obj, 0))))

#define atomic_load_explicit64(obj, order)			\
	(order == memory_order_relaxed				\
	 ? InterlockedOr64NoFence(obj, 0)			\
	 : (order == memory_order_acquire			\
	    ? InterlockedOr64Acquire(obj, 0)			\
	    : (order == memory_order_release			\
	       ? InterlockedOr64Release(obj, 0)			\
...
atomic_compare_exchange_strong_explicit64(atomic_int_fast64_t *obj,
	...
	switch (succ) {
	case memory_order_relaxed:
		__v = InterlockedCompareExchange64NoFence(obj, desired, *expected);
		break;
	case memory_order_acquire:
		__v = InterlockedCompareExchange64Acquire(obj, desired, *expected);
		break;
	case memory_order_release:
		__v = InterlockedCompareExchange64Release(obj, desired, *expected);
		break;
	default:
