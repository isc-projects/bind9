...
isc_result_t isc__mutex_init(isc_mutex_t *mp, const char *file,
                             unsigned int line);
#endif
#endif
...
#define isc_mutex_destroy(mp)                                                  \
	((pthread_mutex_destroy((&(mp)->mutex)) == 0) ? ISC_R_SUCCESS          \
	                                              : ISC_R_UNEXPECTED)
#else
#define isc_mutex_destroy(mp)                                                  \
...
isc_result_t isc_mutex_init_profile(isc_mutex_t *mp, const char *_file,
                                    int _line);
isc_result_t isc_mutex_lock_profile(isc_mutex_t *mp, const char *_file,
                                    int _line);
isc_result_t isc_mutex_unlock_profile(isc_mutex_t *mp, const char *_file,
                                      int _line);
