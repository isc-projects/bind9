		  ...
		  isc_mem_t **ctxp, unsigned int flags)
{
	...
	ctx->flags = flags;
	(void)isc_refcount_init(&ctx->references, 1);
...
print_contexts(FILE *file) {
	...
	{
		fprintf(file, "context: %p (%s): %" PRId32 " references\n",
			ctx,
...
isc_mem_references(isc_mem_t *ctx0) {
	isc__mem_t *ctx = (isc__mem_t *)ctx0;
	isc_refcount_t refs = isc_refcount_current(&ctx->references);
	return (refs);
}
	      ...
	      xmlTextWriterPtr writer)
{
	...
	TRY0(xmlTextWriterWriteFormatString(writer, "%" PRId32, isc_refcount_current(&ctx->references)));
