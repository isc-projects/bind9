...
isc_result_t
isc_hmac_init(isc_hmac_t *hmac, const void *key, size_t keylen, isc_md_type_t type) {
	REQUIRE(hmac != NULL);
...
isc_md_type_t
isc_hmac_get_type(isc_hmac_t *hmac) {
	REQUIRE(hmac != NULL);

	return (EVP_MD_type(HMAC_CTX_get_md(hmac)));
}

int
isc_hmac_get_digestlen(isc_hmac_t *hmac) {
	REQUIRE(hmac != NULL);
