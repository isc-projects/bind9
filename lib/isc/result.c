...
register_table(resulttable_list_t *tables, unsigned int base,
               unsigned int nresults, const char **text, isc_msgcat_t *msgcat,
               int set)
{
	resulttable *table;
...
initialize_action(void)
{
	...
	result = register_table(&description_tables, ISC_RESULTCLASS_ISC,
	                        ISC_R_NRESULTS, description, isc_msgcat,
	                        ISC_RESULT_RESULTSET);
	if (result != ISC_R_SUCCESS)
		UNEXPECTED_ERROR(__FILE__, __LINE__, "register_table() %s: %u",
		                 isc_msgcat_get(isc_msgcat, ISC_MSGSET_GENERAL,
		                                ISC_MSG_FAILED, "failed"),
		                 result);

	result = register_table(&identifier_tables, ISC_RESULTCLASS_ISC,
	                        ISC_R_NRESULTS, identifier, isc_msgcat,
	                        ISC_RESULT_RESULTSET);
	if (result != ISC_R_SUCCESS)
		UNEXPECTED_ERROR(__FILE__, __LINE__, "register_table() %s: %u",
		                 isc_msgcat_get(isc_msgcat, ISC_MSGSET_GENERAL,
...
isc_result_tomany_helper(resulttable_list_t *tables, isc_result_t result)
{
	...
	if (text == NULL)
		text = isc_msgcat_get(isc_msgcat, ISC_RESULT_UNAVAILABLESET, 1,
                       ...
                       const char **ids, isc_msgcat_t *msgcat, int set)
{
	...
	return (register_table(&identifier_tables, base, nresults, ids, msgcat,
	                       set));
}
