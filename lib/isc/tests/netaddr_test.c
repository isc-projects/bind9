...
ATF_TC_BODY(isc_netaddr_isnetzero, tc)
{
	...
	} tests[] = {{"0.0.0.0", ISC_TRUE},    {"0.0.0.1", ISC_TRUE},
	             {"0.0.1.2", ISC_TRUE},    {"0.1.2.3", ISC_TRUE},
	             {"10.0.0.0", ISC_FALSE},  {"10.9.0.0", ISC_FALSE},
	             {"10.9.8.0", ISC_FALSE},  {"10.9.8.7", ISC_FALSE},
	             {"127.0.0.0", ISC_FALSE}, {"127.0.0.1", ISC_FALSE}};
	...
	for (i = 0; i < sizeof(tests) / sizeof(tests[0]); i++) {
		...
		ATF_CHECK_EQ_MSG(result, tests[i].expect, "%s",
		                 tests[i].address);
	}
}
