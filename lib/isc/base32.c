...
base32_decode_char(base32_decode_ctx_t *ctx, int c)
{
	...
	if (last == 32 && ctx->seen_32 == 0)
		switch (ctx->digits) {
		case 0:
		case 1: return (ISC_R_BADBASE32);
		case 2:
			if ((ctx->val[1] & 0x03) != 0)
				return (ISC_R_BADBASE32);
			ctx->seen_32 = 1;
			break;
		case 3: return (ISC_R_BADBASE32);
		case 4:
			if ((ctx->val[3] & 0x0f) != 0)
				return (ISC_R_BADBASE32);
			ctx->seen_32 = 3;
...
static isc_result_t
base32_decoderegion(isc_region_t *source, const char base[], isc_boolean_t pad,
                    isc_buffer_t *target)
{
	base32_decode_ctx_t ctx;
