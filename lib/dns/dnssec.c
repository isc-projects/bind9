		...
		isc_mem_t *mctx, isc_buffer_t *buffer, dns_rdata_t *sigrdata)
{
	 ...
	 */
	ret = digest_sig(ctx, true, &tmpsigrdata, &sig);
	if (ret != ISC_R_SUCCESS)
...
dns_dnssec_keyactive(dst_key_t *key, isc_stdtime_t now) {
	...
	bool pubset = true, actset = true;
	bool revset = true, inactset = true;
	bool delset = true;
	int major, minor;
		     ...
		     dns_dnsseckey_t **dkp)
{
	...
	dk->key = *dstkey;
	*dstkey = NULL;
	dk->force_publish = true;
	dk->force_sign = true;
	dk->hint_publish = true;
	dk->hint_sign = true;
	dk->hint_remove = true;
	dk->first_sign = true;
	dk->is_active = true;
			     ...
			     dns_dnsseckeylist_t *keylist)
{
	...
	RETERR(dns_name_tofilenametext(origin, true, &b));
