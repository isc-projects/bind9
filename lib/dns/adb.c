...
grow_entries(isc_task_t *task, isc_event_t *ev) {
	...
	/*
	 * Only on success do we set adb->growentries_sent to true.
...
grow_names(isc_task_t *task, isc_event_t *ev) {
	...
	/*
	 * Only on success do we set adb->grownames_sent to true.
	 * This will prevent us being continuously being called on error.
	 */
		   ...
		   unsigned int options, int *bucketp)
{
	...
	bucket = dns_name_fullhash(name, true) % adb->nnames;
		    ...
		    dns_adbname_t *name, isc_stdtime_t now)
{
	...
	if (find->options & DNS_ADBFIND_INET) {
		...
		while (namehook != NULL) {
			 ...
			 */
			inc_entry_refcnt(adb, entry, true);
		    ...
		    dns_adbfind_t **findp)
{
	...
	if (adb->name_sd[bucket]) {
		...
		RUNTIME_CHECK(free_adbfind(adb, &find) == true);
	 ...
	 */
	RUNTIME_CHECK(check_expire_namehooks(adbname, now) == true);
...
dns_adb_dump(dns_adb_t *adb, FILE *f) {
	...
	for (i = 0; i < adb->nnames; i++)
		RUNTIME_CHECK(cleanup_names(adb, i, now) == true);
	for (i = 0; i < adb->nentries; i++)
		RUNTIME_CHECK(cleanup_entries(adb, i, now) == true);

	dump_adb(adb, f, true, now);
	UNLOCK(&adb->lock);
}
...
dns_adb_freeaddrinfo(dns_adb_t *adb, dns_adbaddrinfo_t **addrp) {
	...
	*addrp = NULL;
	...
	want_check_exit = dec_entry_refcnt(adb, overmem, entry, true);
...
dns_adb_flush(dns_adb_t *adb) {
	 ...
	 */
	for (i = 0; i < adb->nnames; i++)
		RUNTIME_CHECK(cleanup_names(adb, i, INT_MAX) == true);
	for (i = 0; i < adb->nentries; i++)
		RUNTIME_CHECK(cleanup_entries(adb, i, INT_MAX) == true);
...
dns_adb_flushname(dns_adb_t *adb, const dns_name_t *name) {
	...
	LOCK(&adb->lock);
	bucket = dns_name_hash(name, true) % adb->nnames;
	LOCK(&adb->namelocks[bucket]);
	adbname = ISC_LIST_HEAD(adb->names[bucket]);
	while (adbname != NULL) {
