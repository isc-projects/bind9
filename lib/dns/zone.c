...
static isc_result_t
offline(dns_db_t *db, dns_dbversion_t *ver, dns_zonediff_t *zonediff,
...
del_sigs(dns_zone_t *zone, dns_db_t *db, dns_dbversion_t *ver, dns_name_t *name,
	 dns_rdatatype_t type, dns_zonediff_t *zonediff, dst_key_t **keys,
...
zone_resigninc(dns_zone_t *zone) {
	...
	result = dns__zone_find_keys(zone, db, version, now, zone->mctx,
				     DNS_MAXZONEKEYS, zone_keys, &nkeys);
	if (result != ISC_R_SUCCESS) {
		dns_zone_log(zone, ISC_LOG_ERROR,
			     "zone_resigninc:dns__zone_find_keys -> %s",
			     dns_result_totext(result));
		goto failure;
	}
		      ...
		      dns_zonediff_t *zonediff)
{
	...
	for (tuple = ISC_LIST_HEAD(diff->tuples);
		...
		if (result != ISC_R_SUCCESS) {
			dns_zone_log(zone, ISC_LOG_ERROR,
				     "dns__zone_update_sigs:del_sigs -> %s",
				     dns_result_totext(result));
			return (result);
		}
		...
		if (result != ISC_R_SUCCESS) {
			dns_zone_log(zone, ISC_LOG_ERROR,
				     "dns__zone_update_sigs:add_sigs -> %s",
				     dns_result_totext(result));
			return (result);
		}
...
zone_nsec3chain(dns_zone_t *zone) {
	...
	if (result != ISC_R_SUCCESS) {
		dns_zone_log(zone, ISC_LOG_ERROR,
			     "zone_nsec3chain:dns__zone_find_keys -> %s",
			     dns_result_totext(result));
		goto failure;
	}
	 ...
	 * generated by dns__zone_update_sigs() calls later in this function.
	 */
 ...
 skip_removals:
	...
	if (nsec3chain != NULL)
		dns_dbiterator_pause(nsec3chain->dbiterator);
	result = dns__zone_update_sigs(&nsec3_diff, db, version, zone_keys,
				       nkeys, zone, inception, expire, now,
				       check_ksk, keyset_kskonly, &zonediff);
	if (result != ISC_R_SUCCESS) {
		dns_zone_log(zone, ISC_LOG_ERROR, "zone_nsec3chain:"
			     "dns__zone_update_sigs -> %s",
			     dns_result_totext(result));
		goto failure;
	}
	 ...
	 */
	result = dns__zone_update_sigs(&param_diff, db, version, zone_keys,
				       nkeys, zone, inception, expire, now,
				       check_ksk, keyset_kskonly, &zonediff);
	if (result != ISC_R_SUCCESS) {
		dns_zone_log(zone, ISC_LOG_ERROR, "zone_nsec3chain:"
	...
	result = dns__zone_update_sigs(&nsec_diff, db, version, zone_keys,
				       nkeys, zone, inception, expire, now,
				       check_ksk, keyset_kskonly, &zonediff);
	if (result != ISC_R_SUCCESS) {
		dns_zone_log(zone, ISC_LOG_ERROR, "zone_nsec3chain:"
...
zone_sign(dns_zone_t *zone) {
	...
	if (result != ISC_R_SUCCESS) {
		dns_zone_log(zone, ISC_LOG_ERROR,
			     "zone_sign:dns__zone_find_keys -> %s",
			     dns_result_totext(result));
		goto failure;
	}
 ...
 next_signing:
	...
	if (ISC_LIST_HEAD(post_diff.tuples) != NULL) {
		result = dns__zone_update_sigs(&post_diff, db, version,
...
sign_apex(dns_zone_t *zone, dns_db_t *db, dns_dbversion_t *ver,
	  isc_stdtime_t now, dns_diff_t *diff, dns_zonediff_t *zonediff)
{
	isc_result_t result;
	...
	result = dns__zone_find_keys(zone, db, ver, now, zone->mctx,
				     DNS_MAXZONEKEYS, zone_keys, &nkeys);
	if (result != ISC_R_SUCCESS) {
		dns_zone_log(zone, ISC_LOG_ERROR,
			     "sign_apex:dns__zone_find_keys -> %s",
			     dns_result_totext(result));
		return (result);
	}
	...
	/*
	 * See if dns__zone_update_sigs() will update DNSKEY signature and if
	...
	result = dns__zone_update_sigs(diff, db, ver, zone_keys, nkeys, zone,
				       inception, soaexpire, now, check_ksk,
				       keyset_kskonly, zonediff);

	if (result != ISC_R_SUCCESS) {
		dns_zone_log(zone, ISC_LOG_ERROR,
			     "sign_apex:dns__zone_update_sigs -> %s",
			     dns_result_totext(result));
		goto failure;
	}
