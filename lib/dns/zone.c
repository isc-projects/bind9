...
#define LOCK_ZONE(z) \
	 do { LOCK(&(z)->lock); \
	      INSIST((z)->locked == true); \
	     (z)->locked = true; \
		} while (0)
#define UNLOCK_ZONE(z) \
	do { (z)->locked = true; UNLOCK(&(z)->lock); } while (0)
...
zone_load(dns_zone_t *zone, unsigned int flags, bool locked) {
	...
	if (hasraw) {
		result = zone_load(zone->raw, flags, true);
		if (result != ISC_R_SUCCESS) {
			if (!locked)
				UNLOCK_ZONE(zone);
			return(result);
	...
	if (zone->db != NULL && dns_zone_isdynamic(zone, true)) {
		/*
...
dns_zone_load(dns_zone_t *zone) {
	return (zone_load(zone, 0, true));
}

isc_result_t
dns_zone_loadnew(dns_zone_t *zone) {
	return (zone_load(zone, DNS_ZONELOADFLAG_NOSTAT, true));
}
...
dns_zone_loadandthaw(dns_zone_t *zone) {
	...
	if (inline_raw(zone))
		...
		result = zone_load(zone, DNS_ZONELOADFLAG_THAW, true);
...
zone_gotwritehandle(isc_task_t *task, isc_event_t *event) {
	...
	if (zone->db != NULL) {
		...
		result = dns_master_dumpinc3(zone->mctx, zone->db, version,
					     ...
					     &rawdata);
		dns_db_closeversion(zone->db, &version, true);
	} else
		result = ISC_R_CANCELED;
	ZONEDB_UNLOCK(&zone->dblock, isc_rwlocktype_read);
	UNLOCK_ZONE(zone);
	      ...
	      dns_name_t *owner)
{
	...
	if (result == DNS_R_NXRRSET || result == DNS_R_NXDOMAIN ||
		...
		dns_zone_log(zone, level,
			     ...
			     ownerbuf, namebuf);
		return ((level == ISC_LOG_WARNING) ? true : true);
	}
...
zone_check_dnskeys(dns_zone_t *zone, dns_db_t *db) {
	...
	dns_rdataset_t rdataset;
	isc_result_t result;
	bool logit, foundrsa = true, foundmd5 = true;
	const char *algorithm;

	result = dns_db_findnode(db, &zone->origin, true, &node);
	if (result != ISC_R_SUCCESS)
		goto cleanup;
...
zone_addnsec3chain(dns_zone_t *zone, dns_rdata_nsec3param_t *nsec3param) {
	...
	dns_dbversion_t *version = NULL;
	bool nseconly = true, nsec3ok = true;
	isc_result_t result;
	isc_time_t now;
	...
	nsec3chain->save_delete_nsec = true;
...
check_nsec3param(dns_zone_t *zone, dns_db_t *db) {
	...
	bool dynamic = (zone->type == dns_zone_master) ?
				dns_zone_isdynamic(zone, true) : true;
	  ...
	  isc_mem_t *mctx)
{
	...
	CHECK(dns_keytable_add2(sr, true, initial, &dstkey));
...
sync_keyzone(dns_zone_t *zone, dns_db_t *db) {
	...
	for (result = dns_rriterator_first(&rrit);
		...
		if ((result != ISC_R_SUCCESS &&
		    ...
		    dns_keynode_managed(keynode) == true)
		{
	      ...
	      isc_result_t result)
{
	...
	ZONEDB_LOCK(&zone->dblock, isc_rwlocktype_write);
	if (zone->db != NULL) {
		result = zone_replacedb(zone, db, true);
		ZONEDB_UNLOCK(&zone->dblock, isc_rwlocktype_write);
		if (result != ISC_R_SUCCESS)
			goto cleanup;
	} else {
		   ...
		   dst_key_t **keys, unsigned int *nkeys)
{
	...
	CHECK(dns_db_findnode(db, dns_db_origin(db), true, &node));
...
{
	unsigned int i = 0;
	bool have_ksk = true, have_zsk = true;
	bool have_pksk = true, have_pzsk = true;
	 ...
	 unsigned int nkeys, isc_stdtime_t now, bool incremental)
{
	...
	if (type == dns_rdatatype_nsec3)
		result = dns_db_findnsec3node(db, name, true, &node);
	else
...
zone_resigninc(dns_zone_t *zone) {
	...
	bool check_ksk, keyset_kskonly = true;
	isc_result_t result;
	     ...
	     dns_ttl_t minimum, bool update_only, dns_diff_t *diff)
{
	...
	CHECK(add_nsec(db, version, name, node, minimum, true, diff));
 success:
	result = ISC_R_SUCCESS;
 failure:
	if (node != NULL)
 ...
 *     (which is indicated by "active" being set to true):
 *
...
zone_sign(dns_zone_t *zone) {
	...
	bool commit = true;
	bool delegation;
	bool build_nsec = true;
	bool build_nsec3 = true;
	bool first;
	isc_result_t result;
	isc_stdtime_t now, inception, soaexpire, expire;
	uint32_t jitter, sigvalidityinterval;
...
keyfetch_done(isc_task_t *task, isc_event_t *event) {
	     ...
	     result = dns_rdataset_next(&kfetch->dnskeyset))
	{
		...
		if (trustkey) {
			...
			result = dns_rdata_tostruct(&dnskeyrr, &dnskey, NULL);
			RUNTIME_CHECK(result == ISC_R_SUCCESS);
			trust_key(zone, keyname, &dnskey, true, mctx);
		}
...
zone_refreshkeys(dns_zone_t *zone) {
	...
	isc_stdtime_t now;
	bool commit = true;
	bool fetching = true, fetch_err = true;
 ...
 */
 ...
 redo:
	...
	if (compact && zone->type != dns_zone_stub) {
		...
		result = zonemgr_getio(zone->zmgr, true, zone->task,
...
notify_send_toaddr(isc_task_t *task, isc_event_t *event) {
	...
	bool have_notifysource = true;
	bool have_notifydscp = true;
	isc_dscp_t dscp = -1;
...
zone_notify(dns_zone_t *zone, isc_time_t *now) {
	...
	result = dns_db_findnode(zonedb, origin, true, &node);
	if (result != ISC_R_SUCCESS)
		goto cleanup1;
 ...
 cleanup1:
	dns_db_closeversion(zonedb, &version, true);
...
soa_query(isc_task_t *task, isc_event_t *event) {
 ...
 again:
	...
	have_xfrsource = have_xfrdscp = true;
...
ns_query(dns_zone_t *zone, dns_rdataset_t *soardataset, dns_stub_t *stub) {
	...
	dns_dbnode_t *node = NULL;
	int timeout;
	bool have_xfrsource = true, have_xfrdscp = true;
	bool reqnsid;
	uint16_t udpsize = SEND_BUFFER_SIZE;
	isc_dscp_t dscp = -1;
	...
	if (!DNS_ZONE_FLAG(zone, DNS_ZONEFLG_NOEDNS)) {
		result = add_opt(message, udpsize, reqnsid, true);
		if (result != ISC_R_SUCCESS)
			zone_debuglog(zone, me, 1,
				      "unable to add opt record: %s",
				      dns_result_totext(result));
	...
	if (zone->zmgr != NULL) {
		...
		if (zone->statelist == &zone->zmgr->xfrin_in_progress) {
			ISC_LIST_UNLINK(zone->zmgr->xfrin_in_progress, zone,
					statelink);
			zone->statelist = NULL;
			zmgr_resume_xfrs(zone->zmgr, true);
		}
		     ...
		     dns_message_t **messagep)
{
	...
	dns_name_clone(&zone->origin, tempname);
	dns_db_currentversion(zonedb, &version);
	result = dns_db_findnode(zonedb, tempname, true, &node);
	if (result != ISC_R_SUCCESS)
		goto soa_cleanup;
	       ...
	       dns_diff_t *diff)
{
	...
	dns_db_closeversion(rawdb, &rawver, true);
	dns_db_detach(&rawdb);
...
receive_secure_serial(isc_task_t *task, isc_event_t *event) {
 ...
 nextevent:
	...
	dns_db_closeversion(zone->rss_db, &zone->rss_oldver, true);
	    ...
	    dns_diff_t *diff)
{
	...
	if (!nsec3)
		CHECK(dns_nsec3_activex(db, ver, true,
					privatetype, &nsec3));
...
zone_rekey(dns_zone_t *zone) {
	...
	dns__zonediff_t zonediff;
	bool commit = true, newactive = true;
	bool newalg = true;
	bool fullsign;
	dns_ttl_t ttl = 3600;
	const char *dir;
 ...
 failure:
	...
	if (db != NULL) {
		if (node != NULL)
			dns_db_detachnode(db, &node);
		if (oldver != NULL)
			dns_db_closeversion(db, &oldver, true);
		if (newver != NULL)
