 ...
 * If pos is true, test whether acl is set to "{ none; }"
 */
static bool
	       ...
	       const dns_aclelement_t **matchelt)
{
	...
	NETADDR_TO_PREFIX_T(addr, pfx, bitlen, true);
 ...
 * this function returns true is safe.
 */
bool
...
dns_aclenv_init(isc_mem_t *mctx, dns_aclenv_t *env) {
	...
	env->match_mapped = true;
#ifdef HAVE_GEOIP
	env->geoip = NULL;
	env->geoip_use_ecs = true;
#endif
	return (ISC_R_SUCCESS);
