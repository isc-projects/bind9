...
dns_zt_unmount(dns_zt_t *zt, dns_zone_t *zone) {
	...
	result = dns_rbt_deletename(zt->table, name, true);
...
zt_destroy(dns_zt_t *zt) {
	if (zt->flush)
		(void)dns_zt_apply(zt, true, flush, NULL);
	dns_rbt_destroy(&zt->table);
...
dns_zt_asyncload(dns_zt_t *zt, dns_zt_allloaded_t alldone, void *arg) {
	...
	result = dns_zt_apply2(zt, true, NULL, asyncload, &dl);
...
dns_zt_freezezones(dns_zt_t *zt, bool freeze) {
	...
	RWLOCK(&zt->rwlock, isc_rwlocktype_read);
	result = dns_zt_apply2(zt, true, &tresult, freezezones, &freeze);
	RWUNLOCK(&zt->rwlock, isc_rwlocktype_read);
