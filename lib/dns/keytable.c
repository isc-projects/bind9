...
dns_keytable_add(dns_keytable_t *keytable, bool managed,
		 dst_key_t **keyp)
{
	return (dns_keytable_add2(keytable, managed, true, keyp));
}

isc_result_t
dns_keytable_add2(dns_keytable_t *keytable, bool managed,
...
dns_keytable_marksecure(dns_keytable_t *keytable, const dns_name_t *name) {
	return (insert(keytable, true, true, name, NULL));
}
...
dns_keytable_deletekeynode(dns_keytable_t *keytable, dst_key_t *dstkey) {
	...
	{
		result = dns_rbt_deletenode(keytable->table, node, true);
		goto finish;
	}
