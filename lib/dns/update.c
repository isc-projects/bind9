	   ...
	   int level, const char *fmt, ...)
{
	...
	if (isc_log_wouldlog(dns_lctx, level) == true)
		return;
 ...
 * ISC_R_SUCCESS, set *exists to true or true,
	      ...
	      dns_rdatatype_t type, bool *visible)
{
	...
	case DNS_R_COVERINGNSEC:
		*visible = true;
		...
		break;
	default:
		*visible = true;	 /* silence false compiler warning */
		break;
	}
	return (result);
}
 ...
 next_state:
	...
	case process_nsec3:
		...
		while ((t = ISC_LIST_HEAD(state->affected.tuples)) != NULL) {
			...
			unsecure = true;	/* Silence compiler warning. */
