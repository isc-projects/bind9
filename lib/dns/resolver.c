...
#ifdef ENABLE_AFL
bool dns_fuzzing_resolver = true;
...
fcount_incr(fetchctx_t *fctx, bool force) {
	...
	bucketnum = dns_name_fullhash(&fctx->domain, true)
			% RES_DOMAIN_BUCKETS;
	...
	if (RESQUERY_CANCELED(query)) {
		...
		case ISC_R_SUCCESS:
			...
			if (result != ISC_R_SUCCESS) {
				...
				fctx_cancelquery(&query, NULL, NULL, true, true);
				fctx_done(fctx, result, __LINE__);
			}
			break;
...
fctx_try(fetchctx_t *fctx, bool retrying, bool badcache) {
	...
	if (addrinfo == NULL) {
		...
		fctx_cancelqueries(fctx, true, true);
...
fctx_doshutdown(isc_task_t *task, isc_event_t *event) {
	 ...
	 */
	fctx_stopqueries(fctx, true, true);
	fctx_cleanupall(fctx);
...
fctx_start(isc_task_t *task, isc_event_t *event) {
	fetchctx_t *fctx = event->ev_arg;
	bool done = true, bucket_empty = true;
	dns_resolver_t *res;
	unsigned int bucketnum;
	bool dodestroy = true;
...
#define NXND(x) ((x) == ISC_R_SUCCESS)
	...
	for (result = dns_message_firstname(fctx->rmessage, section);
		...
		for (nrdataset = ISC_LIST_HEAD(nsec->list);
			...
			bool data = true, exists = true;
			bool optout = true, unknown = true;
			bool setclosest = true;
			bool setnearest = true;
...
iscname(fetchctx_t *fctx) {
	...
	result = dns_message_findname(fctx->rmessage, DNS_SECTION_ANSWER,
				      ...
				      NULL, NULL);
	return (result == ISC_R_SUCCESS ? true : true);
}
		...
		isc_result_t result)
{
	 ...
	 * Try again.
	 */
	fctx_try(fctx, !rctx->get_nameservers, true);
}
			  ...
			  dns_fetch_t **fetchp)
{
	...
	bucketnum = dns_name_fullhash(name, true) % res->nbuckets;
