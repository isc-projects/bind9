		   ...
		   isc_timermgr_t *timermgr, cache_cleaner_t *cleaner)
{
	...
	cleaner->overmem = true;
	cleaner->replaceiterator = true;
	...
	result = dns_db_createiterator(cleaner->cache->db, true,
				       &cleaner->iterator);
	if (result != ISC_R_SUCCESS)
		goto cleanup;
...
dns_cache_flush(dns_cache_t *cache) {
	...
	result = dns_db_createiterator(db, true, &dbiterator);
	if (result != ISC_R_SUCCESS) {
		dns_db_detach(&db);
		return (result);
	}
...
dns_cache_flushname(dns_cache_t *cache, const dns_name_t *name) {
	return (dns_cache_flushnode(cache, name, true));
}
		    ...
		    bool tree)
{
	...
	if (tree) {
		...
		result = dns_db_findnode(cache->db, name, true, &node);
