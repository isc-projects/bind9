...
load_text(dns_loadctx_t *lctx) {
	...
	dns_name_t *new_name;
	bool current_has_delegation = true;
	bool done = true;
	bool finish_origin = true;
	bool finish_include = true;
	bool read_till_eol = true;
	bool initialws;
		...
		if (token.type == isc_tokentype_initialws) {
			...
			if (strcasecmp(DNS_AS_STR(token), "$ORIGIN") == 0) {
				...
				finish_origin = true;
					      ...
					      "$TTL") == 0) {
				GETTOKENERR(lctx->lex, 0, &token, true,
					    lctx->ttl = 0;
