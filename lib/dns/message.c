	...
	dns_decompress_t *dctx)
{
	...
	tries = 0;
	while (tries < 2) {
		result = dns_name_fromwire(name, source, dctx, true,
					   scratch);
	   ...
	   dns_section_t sectionid, unsigned int options)
{
	...
	dns_ttl_t ttl;
	dns_namelist_t *section;
	bool free_name = true, free_rdataset = true;
	...
	for (count = 0; count < msg->counts[sectionid]; count++) {
		...
		if (seen_problem) {
			...
			if (free_rdataset)
				isc_mempool_put(msg->rdspool, rdataset);
			free_name = free_rdataset = true;
		}
		INSIST(free_name == true);
		INSIST(free_rdataset == true);
	}
