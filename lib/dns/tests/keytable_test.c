...
create_tables() {
	...
	assert_int_equal(dns_keytable_add2(keytable, false, false, &key),
			 ISC_R_SUCCESS);

	/* Add an initializing managed key */
	create_key(257, 3, 5, "managed.com", keystr1, &key);
	assert_int_equal(dns_keytable_add2(keytable, true, true, &key),
			 ISC_R_SUCCESS);
...
add_test(void **state) {
	...
	assert_int_equal(dns_keytable_nextkeynode(keytable, keynode,
						  &next_keynode),
			 ISC_R_SUCCESS);
	assert_true(dns_keynode_initial(keynode));
	dns_keytable_detachkeynode(keytable, &next_keynode);
	dns_keytable_detachkeynode(keytable, &keynode);
	 ...
	 */
	create_key(257, 3, 5, "managed.com", keystr2, &key);
	assert_int_equal(dns_keytable_add2(keytable, true, false, &key),
			 ISC_R_SUCCESS);
	assert_int_equal(dns_keytable_find(keytable, str2name("managed.com"),
					   &keynode),
			 ISC_R_SUCCESS);
	assert_false(dns_keynode_initial(keynode));
	assert_int_equal(dns_keytable_nextkeynode(keytable, keynode,
						  &next_keynode),
			 ISC_R_SUCCESS);
	dns_keytable_detachkeynode(keytable, &keynode);
	...
	assert_true(dns_keynode_key(keynode) != NULL); /* now have a key */
...
nta_test(void **state) {
	...
	result = dns_keytable_add2(keytable, false, false, &key);
	assert_int_equal(result, ISC_R_SUCCESS);
