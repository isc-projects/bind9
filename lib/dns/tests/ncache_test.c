 ...
 */
static void
ncache_add_size_test(void **state) {
	struct {
		size_t size;
 ...
 */
static void
ncache_add_soa_test(void **state) {
	dns_fixedname_t fqname;
	dns_name_t *qname = name_fromstring(&fqname, "missing.example.");
	dns_message_t *msg = NULL;
	isc_result_t result, find_result;
 ...
 */
static void
ncache_add_count_test(void **state) {
	dns_fixedname_t fqname;
	dns_name_t *qname = name_fromstring(&fqname, "missing.example.");
	dns_message_t *msg = negative_response();
	isc_result_t result, find_result;
...
main(void) {
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_setup_teardown(ncache_add_size_test, _setup,
						_teardown),
		cmocka_unit_test_setup_teardown(ncache_add_soa_test, _setup,
						_teardown),
		cmocka_unit_test_setup_teardown(ncache_add_count_test, _setup,
						_teardown),
	};
