	      ...
	      dns_compress_t *cctx, dns_decompress_t *dctx)
{
	...
	RUNTIME_CHECK(dns_name_fromwire(&name, &source, dctx, true,
...
ATF_TC_BODY(istat, tc) {
	...
	struct {
		...
		{ "_ta-12345-abcf-ED89", true },
		{ "_ta-.example", true },
		{ "_ta-1234.example", true },
		{ "_ta-1234-abcf.example", true },
		{ "_ta-1234-abcf-ED89.example", true },
		{ "_ta-12345-abcf-ED89.example", true },
		{ "_ta-1234-abcfe-ED89.example", true },
		{ "_ta-1234-abcf-EcD89.example", true }
	};
...
ATF_TC_BODY(hash, tc) {
	...
	for (i = 0; i < (sizeof(testcases)/sizeof(testcases[0])); i++) {
		...
		h1 = dns_name_hash(n1, true);
		h2 = dns_name_hash(n2, true);
