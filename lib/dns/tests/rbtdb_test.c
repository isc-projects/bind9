...
/* rbtdb.c and dnstest.h both define CHECK */
#undef CHECK
#include "dnstest.h"
 ...
 */
static void
proof_rdataset_survives_expiration_cleanup_test(void **state) {
	isc_result_t result;
	dns_db_t *db = NULL;
	...
	isc_stdtime_get(&now);
	result = isc_mem_create(0, 0, &dbmctx);
	assert_int_equal(result, ISC_R_SUCCESS);
 ...
 */
static void
zone_nodes_outside_zone_test(void **state) {
	isc_result_t result;
	dns_db_t *db = NULL;
...
main(void) {
	const struct CMUnitTest tests[] = {
		cmocka_unit_test_setup_teardown(
			proof_rdataset_survives_expiration_cleanup_test,
			_setup, _teardown),
		cmocka_unit_test_setup_teardown(zone_nodes_outside_zone_test,
						_setup, _teardown),
	};
