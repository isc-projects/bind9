...
start_zt_asyncload(isc_task_t *task, isc_event_t *event) {
	...
	result = dns_zt_asyncload(args->arg1, all_done, args->arg2);
...
start_zone_asyncload(isc_task_t *task, isc_event_t *event) {
	...
	result = dns_zone_asyncload(args->arg1, load_done, args->arg2);
	ATF_REQUIRE_EQ(result, ISC_R_SUCCESS);
	isc_event_free(&event);
}
...
ATF_TC_BODY(asyncload_zone, tc) {
	...
	ATF_CHECK(!done);
	result = dns_zone_setfile(zone, "testdata/zt/zone1.db",
...
ATF_TC_BODY(asyncload_zt, tc) {
	...
	ATF_REQUIRE_EQ(result, ISC_R_SUCCESS);
	result = dns_zone_setfile(zone1, "testdata/zt/zone1.db",
				  dns_masterformat_text,
				  &dns_master_style_default);
	ATF_REQUIRE_EQ(result, ISC_R_SUCCESS);
	...
	ATF_REQUIRE_EQ(result, ISC_R_SUCCESS);
	result = dns_zone_setfile(zone2, "testdata/zt/zone1.db",
				  dns_masterformat_text,
				  &dns_master_style_default);
	ATF_REQUIRE_EQ(result, ISC_R_SUCCESS);
	...
	ATF_REQUIRE_EQ(result, ISC_R_SUCCESS);
	result = dns_zone_setfile(zone3, "testdata/zt/nonexistent.db",
				  dns_masterformat_text,
				  &dns_master_style_default);
	ATF_REQUIRE_EQ(result, ISC_R_SUCCESS);
