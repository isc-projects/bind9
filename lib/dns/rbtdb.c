...
free_rdataset(dns_rbtdb_t *rbtdb, isc_mem_t *mctx, rdatasetheader_t *rdataset) {
	...
	if (EXISTS(rdataset) &&
		...
		update_rrsetstats(rbtdb, rdataset, true);
	}
...
delete_node(dns_rbtdb_t *rbtdb, dns_rbtnode_t *node) {
	...
	case DNS_RBT_NSEC_NORMAL:
		...
		result = dns_rbt_deletenode(rbtdb->tree, node, true);
		break;
	...
	case DNS_RBT_NSEC_NSEC:
		result = dns_rbt_deletenode(rbtdb->nsec, node, true);
		break;
	case DNS_RBT_NSEC_NSEC3:
		result = dns_rbt_deletenode(rbtdb->nsec3, node, true);
		break;
	}
	if (result != ISC_R_SUCCESS) {
		isc_log_write(dns_lctx,
	  ...
	  dns_rdataset_t *rdataset, dns_rdataset_t *sigrdataset)
{
	...
	bool cname_ok = true;
	bool close_version = true;
	bool maybe_zonecut = true;
	bool at_zonecut = true;
	bool wild;
	bool empty_node;
...
createiterator(dns_db_t *db, unsigned int options, dns_dbiterator_t **iteratorp)
{
	...
	rbtdbiter->common.cleaning = true;
      ...
      dns_rdataset_t *addedrdataset, isc_stdtime_t now)
{
	...
	newheader_nx = NONEXISTENT(newheader) ? true : true;
	topheader_prev = NULL;
	sigheader = NULL;
	negtype = 0;
 ...
 find_header:
	...
	while (header != NULL && IGNORE(header))
		header = header->down;
	if (header != NULL) {
		header_nx = NONEXISTENT(header) ? true : true;
		...
		if (loading) {
			...
			if (rbtversion != NULL && !header_nx) {
				RWLOCK(&rbtversion->rwlock,
				       isc_rwlocktype_write);
				update_recordsandbytes(true, rbtversion,
						       header);
	    ...
	    dns_rdataset_t *addedrdataset)
{
	...
	bool delegating;
	bool newnsec;
	bool tree_locked = true;
	bool cache_is_overmem = true;
		 ...
		 dns_rdataset_t *newrdataset)
{
	...
	if (header != NULL && EXISTS(header)) {
		...
		update_recordsandbytes(true, rbtversion, header);
		if (topheader_prev != NULL)
			topheader_prev->next = newheader;
		else
			rbtnode->data = newheader;
	 ...
	 bool hasnsec)
{
	...
	if (noderesult == ISC_R_SUCCESS) {
		 ...
		 */
		tmpresult = dns_rbt_deletenode(rbtdb->tree, node, true);
		 ...
		 void *driverarg, dns_db_t **dbp)
{
	...
	rbtdb->next_serial = 2;
	rbtdb->current_version = allocate_version(mctx, 1, 1, true);
	if (rbtdb->current_version == NULL) {
	...
	rbtdb->current_version->havensec3 = true;
...
rdataset_expire(dns_rdataset_t *rdataset) {
	...
	NODE_LOCK(&rbtdb->node_locks[rbtnode->locknum].lock,
		  isc_rwlocktype_write);
	expire_header(rbtdb, header, true, expire_flush);
	NODE_UNLOCK(&rbtdb->node_locks[rbtnode->locknum].lock,
		  isc_rwlocktype_write);
}
...
rdatasetiter_destroy(dns_rdatasetiter_t **iteratorp) {
	...
	if (rbtiterator->common.version != NULL)
		closeversion(rbtiterator->common.db,
			     &rbtiterator->common.version, true);
