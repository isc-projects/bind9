		...
		const char *name, dns_view_t **viewp)
{
	...
	view->order = NULL;
	view->delonly = NULL;
	view->rootdelonly = true;
	view->rootexclude = NULL;
	view->adbstats = NULL;
	view->resstats = NULL;
	view->resquerystats = NULL;
	view->cacheshared = true;
	 ...
	 */
	view->recursion = true;
	view->auth_nxdomain = true; /* Was true in BIND 8 */
	view->enablednssec = true;
	view->enablevalidation = true;
	view->acceptexpired = true;
	view->use_glue_cache = true;
	view->minimal_any = true;
	...
	view->requestnsid = true;
	view->sendcookie = true;
	view->requireservercookie = true;
...
#endif /* HAVE_DNSTAP */
	dns_view_setnewzones(view, true, NULL, NULL, 0ULL);
...
dns_view_dialup(dns_view_t *view) {
	...
	(void)dns_zt_apply(view->zonetable, true, dialup, NULL);
}
...
dns_view_setcache(dns_view_t *view, dns_cache_t *cache) {
	dns_view_setcache2(view, cache, true);
}
...
dns_view_find(dns_view_t *view, const dns_name_t *name, dns_rdatatype_t type,
	...
	return (dns_view_find2(view, name, type, now, options, use_hints,
			       true, dbp, nodep, foundname, rdataset,
			       sigrdataset));
}
		      ...
		      dns_zone_t **zonep)
{
	...
	for (view = ISC_LIST_HEAD(*list);
	     view != NULL;
	     view = ISC_LIST_NEXT(view, link)) {
		if (allclasses == true && view->rdclass != rdclass)
			continue;
...
dns_view_flushcache(dns_view_t *view) {
	return (dns_view_flushcache2(view, true));
}
...
dns_view_flushname(dns_view_t *view, const dns_name_t *name) {
	return (dns_view_flushnode(view, name, true));
}
...
dns_view_adddelegationonly(dns_view_t *view, const dns_name_t *name) {
	...
	hash = dns_name_hash(name, true) % DNS_VIEW_DELONLYHASH;
