...
typedef enum {
	DNS_ZONEOPT_SERVERS          = 1ULL<<0,  /*%< perform server checks */
	DNS_ZONEOPT_PARENTS          = 1ULL<<1,  /*%< perform parent checks */
	DNS_ZONEOPT_CHILDREN         = 1ULL<<2,  /*%< perform child checks */
	DNS_ZONEOPT_NOTIFY           = 1ULL<<3,  /*%< perform NOTIFY */
	DNS_ZONEOPT_MANYERRORS       = 1ULL<<4,  /*%< return many errors on load */
	DNS_ZONEOPT_IXFRFROMDIFFS    = 1ULL<<5,  /*%< calculate differences */
	DNS_ZONEOPT_NOMERGE          = 1ULL<<6,  /*%< don't merge journal */
	DNS_ZONEOPT_CHECKNS          = 1ULL<<7,  /*%< check if NS's are addresses */
	DNS_ZONEOPT_FATALNS          = 1ULL<<8,  /*%< DNS_ZONEOPT_CHECKNS is fatal */
	DNS_ZONEOPT_MULTIMASTER      = 1ULL<<9,  /*%< this zone has multiple masters */
	DNS_ZONEOPT_USEALTXFRSRC     = 1ULL<<10, /*%< use alternate transfer sources */
	DNS_ZONEOPT_CHECKNAMES       = 1ULL<<11, /*%< check-names */
	DNS_ZONEOPT_CHECKNAMESFAIL   = 1ULL<<12, /*%< fatal check-name failures */
	DNS_ZONEOPT_CHECKWILDCARD    = 1ULL<<13, /*%< check for internal wildcards */
	DNS_ZONEOPT_CHECKMX          = 1ULL<<14, /*%< check-mx */
	DNS_ZONEOPT_CHECKMXFAIL      = 1ULL<<15, /*%< fatal check-mx failures */
	DNS_ZONEOPT_CHECKINTEGRITY   = 1ULL<<16, /*%< perform integrity checks */
	DNS_ZONEOPT_CHECKSIBLING     = 1ULL<<17, /*%< perform sibling glue checks */
	DNS_ZONEOPT_NOCHECKNS        = 1ULL<<18, /*%< disable IN NS address checks */
	DNS_ZONEOPT_WARNMXCNAME      = 1ULL<<19, /*%< warn on MX CNAME check */
	DNS_ZONEOPT_IGNOREMXCNAME    = 1ULL<<20, /*%< ignore MX CNAME check */
	DNS_ZONEOPT_WARNSRVCNAME     = 1ULL<<21, /*%< warn on SRV CNAME check */
	DNS_ZONEOPT_IGNORESRVCNAME   = 1ULL<<22, /*%< ignore SRV CNAME check */
	DNS_ZONEOPT_UPDATECHECKKSK   = 1ULL<<23, /*%< check dnskey KSK flag */
	DNS_ZONEOPT_TRYTCPREFRESH    = 1ULL<<24, /*%< try tcp refresh on udp failure */
	DNS_ZONEOPT_NOTIFYTOSOA      = 1ULL<<25, /*%< Notify the SOA MNAME */
	DNS_ZONEOPT_NSEC3TESTZONE    = 1ULL<<26, /*%< nsec3-test-zone */
	DNS_ZONEOPT_SECURETOINSECURE = 1ULL<<27, /*%< dnssec-secure-to-insecure */
	DNS_ZONEOPT_DNSKEYKSKONLY    = 1ULL<<28, /*%< dnssec-dnskey-kskonly */
	DNS_ZONEOPT_CHECKDUPRR       = 1ULL<<29, /*%< check-dup-records */
	DNS_ZONEOPT_CHECKDUPRRFAIL   = 1ULL<<30, /*%< fatal check-dup-records failures */
	DNS_ZONEOPT_CHECKSPF         = 1ULL<<31, /*%< check SPF records */
	DNS_ZONEOPT_CHECKTTL         = 1ULL<<32, /*%< check max-zone-ttl */
	DNS_ZONEOPT_AUTOEMPTY        = 1ULL<<33 /*%< automatic empty zone */
} dns_zoneopt_t;
/*
