...
dns_rbt_destroy2(dns_rbt_t **rbtp, unsigned int quantum) {
	...
	deletetreeflat(rbt, quantum, true, &rbt->root);
	if (rbt->root != NULL)
		return (ISC_R_QUOTA);
	...
	while (ISC_LIKELY(current != NULL)) {
		...
		hashagain:
			...
			dns_name_getlabelsequence(name,
						  ...
						  hlabels + tlabels,
						  &hash_name);
			hash = dns_name_fullhash(&hash_name, true);
...
hash_add_node(dns_rbt_t *rbt, dns_rbtnode_t *node, const dns_name_t *name) {
	...
	HASHVAL(node) = dns_name_fullhash(name, true);
			 ...
			 dns_name_t *origin, dns_rbtnode_t **node)
{
	...
	if (origin != NULL) {
		if (chain->level_count > 0)
			result = chain_name(chain, origin, true);
		else
