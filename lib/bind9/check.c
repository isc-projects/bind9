		    ...
		    isc_log_t *logctx, isc_mem_t *mctx)
{
	...
	for (i = 0; acls[i] != NULL; i++) {
		...
		if (recursion == true && !dns_acl_isnone(acl)) {
		 ...
		 uint32_t *countp, isc_log_t *logctx, isc_mem_t *mctx)
{
	...
	REQUIRE(countp != NULL);
	result = isc_symtab_create(mctx, 100, NULL, NULL, true, &symtab);
	if (result != ISC_R_SUCCESS) {
		*countp = count;
		return (result);
	}
	       ...
	       isc_log_t *logctx, isc_mem_t *mctx)
{
	     ...
	     element = cfg_list_next(element))
	{
		...
		for (element2 = cfg_list_first(keylist);
			...
			tresult = check_trusted_key(obj, true, &tflags,
						    logctx);
			if (tresult != ISC_R_SUCCESS)
				result = tresult;
		}
		      ...
		      isc_mem_t *mctx)
{
	 ...
	 */
	tresult = isc_symtab_create(mctx, 100, NULL, NULL, true,
				    &files);
	if (tresult != ISC_R_SUCCESS) {
		result = tresult;
		goto cleanup;
