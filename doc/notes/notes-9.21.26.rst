...
- Remove the :rfc:`1918` reverse-lookup leakage warning.
  ...
  When default empty zones were disabled, :iscman:`named` would log an
...
- Reject out-of-zone records in zone transfers and zone files.
  ...
  the saved zone file; the :iscman:`dnssec-signzone` and
  :iscman:`dnssec-verify` utilities were similarly lenient with zone
  files. Zone transfers, saved secondary zone files, and zone files
  processed by the DNSSEC tools are now all rejected as malformed when
  they contain out-of-zone records. :gl:`#5842`

Bug Fixes
~~~~~~~~~

- Prevent a crash when using both :any:`dns64` and :any:`filter-a`.

  An assertion failure was possible when using both :any:`dns64` and the
  :any:`filter-a` plugin simultaneously; this has been fixed.
  :gl:`#5979`
...
- Fix multiple configuration parser crashes.
  ...
  Two kinds of invalid configuration could cause
  :iscman:`named-checkconf` and :iscman:`named` to abort instead of
  failing with an error message: a zone with an invalid name whose
  ``file`` clause contained the ``$`` or ``%`` template characters,
  and a configuration file truncated by an unexpected end of file.
  The former could also terminate a running :iscman:`named` during
  reconfiguration. Both cases are now reported as normal configuration
  errors. :gl:`#6363` :gl:`#6364`

- Fix shutdown crash when DLZ instances are in use.

  Previously, :iscman:`named` could crash during shutdown when DLZ
  instances were in use, because of a use-after-free. This is now fixed.
  :gl:`!12644`
...
- Prevent resolver crashes while processing DNS over TCP.
  ...
  This has been fixed. :gl:`!12536`

- Return SERVFAIL when querying a resolver with RD=0 and no cache hit.

  Previously, when a resolver was queried with RD=0 and there was no cache hit for the
  queried name, NOERROR/NODATA was returned. Now SERVFAIL is returned instead.
