...
resume() {
	...
	MERGE_REQUEST_ID=$(gitlab create_merge_request 1 "${REPLAY_COMMIT_TITLE}" "{source_branch: '${REPLAY_BRANCH}', target_branch: '${TARGET_BRANCH}', remove_source_branch: true}" | grep -Eo "${MERGE_REQUEST_BASE_URI}\([0-9]\)+" | cut -f 7 -d /)
	...
	gitlab accept_merge_request 1 "${MERGE_REQUEST_ID}" "{should_remove_source_branch: true, merge_when_pipeline_succeeds: true, sha: '$REPLAY_HASH'"

	echo "Your new MR !${MERGE_REQUEST_ID} has been set to merge after build succeeds."
