...
resume() {
	...
	MERGE_REQUEST_ID=$(gitlab create_merge_request "${GITLAB_PROJECT_ID}" "${REPLAY_COMMIT_TITLE}" "{source_branch: '${REPLAY_BRANCH}', target_branch: '${TARGET_BRANCH}'}" | grep -Eo "${MERGE_REQUEST_BASE_URI}\([0-9]\)+" | cut -f 7 -d /)
