...
git archive --format=tar $remote $tag version | (
  cd $verdir
  tar xf -
)
...
git archive --format=tar $remote $tag | (
  cd $topdir
  tar xf -
)
...
if test -f lib/bind/configure; then
...
for f in $(find $windirs -type f \( $winnames \) -print); do
...
for f in doc/arm/*.html; do
  if test "$f" -nt doc/arm/Bv9ARM-book.xml; then
    ok=ok
  fi
done
...
if test doc/arm/Bv9ARM-book.xml -nt doc/arm/Bv9ARM.pdf; then
  echo "WARNING: ARM source is newer than the PDF version."
fi

for f in $(find . -name "*.docbook" -print); do
  docbookfile=$f
  htmlfile=${f%.docbook}.html
  if test $docbookfile -nt $htmlfile; then
